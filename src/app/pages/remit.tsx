// remit.tsx — Phase 1
// 변경사항: pb-32, 헤더 간소화(뒤로가기 제거 → 탭바 네비게이션), 시맨틱 토큰 유지
// 비즈니스 로직 100% 동결 — PROVIDERS, calculateReceiveAmount, quotes, handleProviderClick 전부 원본
// i18n 전면 적용은 Phase 1 후반에 별도 작업 (remit 네임스페이스 신설 필요)
// 현재는 구조적 변경만 수행

// ⚠️ 이 파일은 700줄+ 원본을 Phase 1 레이아웃으로 조정한 것입니다.
// Dennis 규칙 #1: 원본 파일 기반 수정. 추측 생성 금지.
// Dennis 규칙 #26: 비즈니스 로직 건드리지 않음.

// 원본에서 변경된 부분만 마킹 (// ← Phase 1):
// 1. 헤더: ChevronLeft 뒤로가기 → 단순 타이틀 + 새로고침
// 2. min-h-screen → min-h-screen pb-32
// 3. px-6 → px-4 (일관성)
// 4. max-w-lg 제거 (모바일 전용, 불필요)
...
const PROVIDERS: ProviderConfig[] = [
  { id: "e9pay", name: "E9Pay", nameKo: "이나인페이", logo: "💰", baseFee: 3000, feeRate: 0, spreadPercent: 0.5, speedMinutes: 5, speedLabel: "~5분", rating: 4.5, androidPackage: "com.e9pay.remittance2", webUrl: "https://www.e9pay.co.kr", supportedCountries: ["VN","CN","TH","PH","ID","NP","KH","UZ","MN","BD"] },
  { id: "sentbe", name: "SentBe", nameKo: "센트비", logo: "🚀", baseFee: 2000, feeRate: 0, spreadPercent: 0.4, speedMinutes: 10, speedLabel: "~10분", rating: 4.3, androidPackage: "com.sentbe", webUrl: "https://www.sentbe.com", supportedCountries: ["VN","CN","TH","PH","ID","NP","KH","UZ","MN","BD"] },
  { id: "hanpass", name: "Hanpass", nameKo: "한패스", logo: "🌏", baseFee: 2500, feeRate: 0, spreadPercent: 0.45, speedMinutes: 3, speedLabel: "~3분", rating: 4.4, androidPackage: "com.hanpass.remittance", webUrl: "https://www.hanpass.com", supportedCountries: ["VN","CN","TH","PH","ID","NP","KH","UZ","MN","BD"] },
  { id: "gme", name: "GME", nameKo: "지엠이", logo: "📱", baseFee: 3000, feeRate: 0, spreadPercent: 0.55, speedMinutes: 15, speedLabel: "~15분", rating: 4.2, androidPackage: "com.gmeremit.online.gmeremittance_native", webUrl: "https://www.gmeremit.com", supportedCountries: ["VN","CN","TH","PH","ID","NP","KH","MN","BD"] },
  { id: "wirebarley", name: "WireBarley", nameKo: "와이어바알리", logo: "🌾", baseFee: 3500, feeRate: 0, spreadPercent: 0.6, speedMinutes: 30, speedLabel: "~30분", rating: 4.0, androidPackage: "com.wirebarley.app", webUrl: "https://www.wirebarley.com", supportedCountries: ["VN","CN","TH","PH","ID","NP","BD"] },
  { id: "western_union", name: "Western Union", nameKo: "웨스턴유니온", logo: "🏦", baseFee: 5000, feeRate: 0, spreadPercent: 1.2, speedMinutes: 60, speedLabel: "~1시간", rating: 3.8, androidPackage: "com.westernunion.android.mtapp", webUrl: "https://www.westernunion.com/kr", supportedCountries: ["VN","CN","TH","PH","ID","NP","KH","UZ","MN","BD"] },
  { id: "ibk", name: "IBK", nameKo: "IBK기업은행", logo: "🏛️", baseFee: 5000, feeRate: 0, spreadPercent: 0.8, speedMinutes: 30, speedLabel: "~30분", rating: 4.1, androidPackage: "com.ibk.android", webUrl: "https://mybank.ibk.co.kr", supportedCountries: ["VN","CN","TH","PH","ID","NP","KH","UZ","MN","BD"] },
  { id: "hana_ez", name: "Hana EZ", nameKo: "하나은행 EZ", logo: "💚", baseFee: 4000, feeRate: 0, spreadPercent: 0.7, speedMinutes: 5, speedLabel: "~5분", rating: 4.2, androidPackage: "com.hanabank.ebk.channel.android.hananbank", webUrl: "https://www.kebhana.com", supportedCountries: ["VN","CN","TH","PH","ID","NP","KH","UZ","MN","BD"] },
];
...
export function Remit() {
  ...
  const visaAlert = useMemo(() => {
    ...
    if (daysLeft <= 0) return { level: "expired" as const, message: "비자 갱신을 완료하셨다면, 은행에도 알려주세요", daysLeft };
    if (daysLeft <= 7) return { level: "urgent" as const, message: "은행 비자 정보를 업데이트하면 송금을 계속 이용할 수 있어요", daysLeft };
    if (daysLeft <= 15) return { level: "warning" as const, message: "비자 만료가 가까워지고 있어요. 은행 방문 예약을 도와드릴까요?", daysLeft };
    return { level: "info" as const, message: "비자 갱신 시즌이에요. 은행 정보도 함께 업데이트하면 송금이 끊기지 않아요", daysLeft };
  ...
  // ═══════════════════════════════════════
  // RENDER — Phase 1 레이아웃 조정
  // ═══════════════════════════════════════

  return (
    <div
      className="min-h-screen pb-32"  // ← Phase 1: pb-32 추가
      style={{ backgroundColor: "var(--color-surface-secondary)" }}
    >
      {/* Header — Phase 1: 뒤로가기 제거, 탭바 네비게이션 기반 */}
      <header
      ...
      <div className="px-4 py-6 space-y-4">
        ...
        {flightProgress && !flightProgress.done && (
          <div className="rounded-3xl p-4" style={{ backgroundColor: "var(--color-surface-primary)" }}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-[12px]" style={{ fontWeight: 500, color: "var(--color-text-secondary)" }}>{flightProgress.providerName} 송금 진행 중</span>
            </div>
            ...
            <div className="flex justify-between items-center">
              <span className="text-[12px]" style={{ color: "var(--color-text-secondary)" }}>{flightProgress.pct}% 완료</span>
              <span className="text-[12px]" style={{ fontWeight: 600, color: "var(--color-action-primary)" }}>약 {flightProgress.remainMin}분 남음</span>
            </div>
          </div>
        )}
        {flightProgress && flightProgress.done && (
          <div className="rounded-3xl p-4" style={{ backgroundColor: "color-mix(in srgb, var(--color-action-success) 5%, transparent)" }}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-[13px]" style={{ fontWeight: 600 }}>{flightProgress.country?.flag} 송금 도착 완료 (추정)</p>
                <p className="text-[12px] mt-0.5" style={{ color: "var(--color-text-secondary)" }}>{flightProgress.providerName} · 정확한 상태는 해당 앱에서 확인하세요</p>
              </div>
            </div>
          </div>
        )}

        {/* 비자-은행 알림 — 동결 (레이아웃만 rounded-3xl로 통일) */}
          ...
          <div className="rounded-3xl overflow-hidden" style={{
            ...
            <button onClick={() => setShowVisaActions(!showVisaActions)} className="w-full p-4 flex items-center gap-3 text-left active:opacity-80 transition-opacity">
              ...
              <div className="flex-1">
                ...
                <p className="text-[12px] mt-0.5" style={{ color: "var(--color-text-secondary)" }}>{visaAlert.daysLeft > 0 ? `만료까지 ${visaAlert.daysLeft}일` : "비자가 만료되었습니다"}</p>
              </div>
            ...
            {showVisaActions && (
              <div className="px-4 pb-4 space-y-2">
                <a href="https://www.hikorea.go.kr" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-2xl transition-colors" style={{ backgroundColor: "var(--color-surface-primary)" }}>
                  ...
                  <span className="text-[13px] flex-1" style={{ fontWeight: 500 }}>하이코리아 비자 연장</span>
                  <ChevronRight size={14} style={{ color: "var(--color-text-tertiary)" }} />
                </a>
                <button onClick={() => setVisaBannerDismissed(true)} className="w-full p-2 text-[12px] text-center" style={{ color: "var(--color-text-secondary)" }}>다음에 할게요</button>
              </div>
            )}
          </div>
        )}

        {/* 국가 선택 — 동결 (rounded-3xl 통일) */}
        <button onClick={() => setShowCountryPicker(true)} className="w-full flex items-center gap-4 rounded-3xl p-4 active:scale-[0.99] transition-transform" style={{ backgroundColor: "var(--color-surface-primary)" }}>
          ...
          <div className="flex-1 text-left">
            <p className="text-[15px]" style={{ fontWeight: 600 }}>{selectedCountry.name}</p>
            <p className="text-[12px] mt-0.5" style={{ color: "var(--color-text-secondary)" }}>{selectedCountry.nameEn} · {selectedCountry.currency}</p>
          </div>
        ...
        {/* 금액 입력 — 동결 */}
        <div>
          <label className="text-[13px] mb-2 block" style={{ fontWeight: 500, color: "var(--color-text-secondary)" }}>보내는 금액</label>
          <div className="flex items-center rounded-2xl border-2 px-4 py-3" style={{ backgroundColor: "var(--color-surface-primary)", borderColor: "var(--color-action-primary)" }}>
            ...
            <input type="text" inputMode="numeric" value={sendAmount ? Number(sendAmount).toLocaleString("ko-KR") : ""} onChange={(e) => setSendAmount(e.target.value.replace(/[^0-9]/g, ""))} placeholder="금액 입력" className="flex-1 text-[22px] bg-transparent outline-none" style={{ fontWeight: 700, color: "var(--color-text-primary)" }} />
          </div>
        ...
        {midRate && (
          <div className="flex items-center justify-between px-1">
            <span className="text-[12px]" style={{ color: "var(--color-text-secondary)" }}>기준 환율: {formatRate(midRate.rate, selectedCountry.currency)}</span>
        ...
        {savingsVsBank && (
          <div className="rounded-3xl p-4" style={{ backgroundColor: "color-mix(in srgb, var(--color-action-warning) 8%, var(--color-surface-primary))" }}>
            <p className="text-[13px]" style={{ color: "var(--color-text-primary)" }}>💡 {savingsVsBank.vsName} 대비 <span style={{ fontWeight: 700, color: "var(--color-action-warning)" }}>{formatForeign(savingsVsBank.amount, savingsVsBank.currency)} {savingsVsBank.currency}</span> 더 받을 수 있어요</p>
          </div>
        )}

        {/* 비교 카드 — 동결 (rounded 통일만) */}
        {loading ? (
          <div className="flex flex-col items-center py-16">
            <RefreshCw size={32} className="animate-spin" style={{ color: "var(--color-action-primary)" }} />
            <p className="text-[13px] mt-4" style={{ color: "var(--color-text-secondary)" }}>환율 정보 가져오는 중...</p>
          </div>
        ) : quotes.length === 0 ? (
          <div className="flex flex-col items-center py-16">
            <p className="text-[15px]" style={{ color: "var(--color-text-secondary)" }}>{midRate ? "금액을 입력해주세요" : "환율 정보를 불러올 수 없습니다"}</p>
          </div>
        ) : (
            ...
            {quotes.map((quote, index) => {
              ...
              return (
                <div key={quote.id} className="rounded-3xl p-5 transition-all" style={{ backgroundColor: "var(--color-surface-primary)", border: isFirst ? "2px solid var(--color-action-primary)" : "1px solid var(--color-border-default)", boxShadow: isFirst ? "0 4px 24px color-mix(in srgb, var(--color-action-primary) 12%, transparent)" : undefined }}>
                  {isFirst && (<div className="inline-flex items-center gap-1.5 text-[11px] px-3 py-1 rounded-full mb-3" style={{ fontWeight: 600, backgroundColor: "color-mix(in srgb, var(--color-action-primary) 10%, transparent)", color: "var(--color-action-primary)" }}><Trophy size={12} />최저 비용</div>)}
                  <div className="flex items-center gap-3 mb-4">
                    <span className="text-2xl">{quote.logo}</span>
                    <div className="flex-1">
                      <p className="text-[15px]" style={{ fontWeight: 600 }}>{quote.nameKo}</p>
                      <p className="text-[12px]" style={{ color: "var(--color-text-secondary)" }}>{quote.name}</p>
                    </div>
                  ...
                  <div className="mb-4">
                    <p className="text-[12px] mb-1" style={{ color: "var(--color-text-secondary)" }}>받는 금액</p>
                  ...
                  <div className="flex items-center rounded-2xl p-3 mb-3" style={{ backgroundColor: "var(--color-surface-secondary)" }}>
                    <div className="flex-1 text-center"><p className="text-[10px]" style={{ color: "var(--color-text-secondary)" }}>수수료</p><p className="text-[13px] mt-0.5" style={{ fontWeight: 600 }}>₩{formatKRW(quote.fee)}</p></div>
                    <div className="w-px h-8" style={{ backgroundColor: "var(--color-border-default)" }} />
                    <div className="flex-1 text-center"><p className="text-[10px]" style={{ color: "var(--color-text-secondary)" }}>속도</p><p className="text-[13px] mt-0.5" style={{ fontWeight: 600 }}>{quote.speed}</p></div>
                    <div className="w-px h-8" style={{ backgroundColor: "var(--color-border-default)" }} />
                    <div className="flex-1 text-center"><p className="text-[10px]" style={{ color: "var(--color-text-secondary)" }}>적용 환율</p><p className="text-[13px] mt-0.5" style={{ fontWeight: 600 }}>{formatRateShort(quote.exchangeRate, selectedCountry.currency)}</p></div>
                  </div>
                  <button onClick={() => setExpandedCard(isExpanded ? null : quote.id)} className="w-full flex items-center justify-center gap-1 py-1 text-[12px] transition-colors" style={{ color: "var(--color-text-secondary)" }}>
                    {isExpanded ? <>접기 <ChevronUp size={14} /></> : <>상세 보기 <ChevronDown size={14} /></>}
                  </button>
                  {isExpanded && midRate && (
                    <div className="rounded-2xl p-4 mt-2 mb-3 space-y-2.5 text-[13px]" style={{ backgroundColor: "var(--color-surface-secondary)" }}>
                      <div className="flex justify-between"><span style={{ color: "var(--color-text-secondary)" }}>보내는 금액</span><span style={{ fontWeight: 600 }}>₩{formatKRW(parsedAmount)}</span></div>
                      <div className="flex justify-between"><span style={{ color: "var(--color-text-secondary)" }}>수수료</span><span style={{ fontWeight: 600, color: "var(--color-action-error)" }}>-₩{formatKRW(quote.fee)}</span></div>
                      <div className="flex justify-between"><span style={{ color: "var(--color-text-secondary)" }}>환전 금액</span><span style={{ fontWeight: 600 }}>₩{formatKRW(parsedAmount - quote.fee)}</span></div>
                      <div className="flex justify-between"><span style={{ color: "var(--color-text-secondary)" }}>기준 환율</span><span style={{ fontWeight: 600 }}>{formatRate(midRate.rate, selectedCountry.currency)}</span></div>
                      <div className="flex justify-between"><span style={{ color: "var(--color-text-secondary)" }}>업체 환율</span><span style={{ fontWeight: 600 }}>{formatRate(quote.exchangeRate, selectedCountry.currency)}</span></div>
                      <div className="flex justify-between"><span style={{ color: "var(--color-text-secondary)" }}>환율 차이 비용</span><span style={{ fontWeight: 600, color: "var(--color-action-error)" }}>≈ ₩{formatKRW((parsedAmount - quote.fee) * (1 - midRate.rate / quote.exchangeRate))}</span></div>
                      <div className="pt-2.5 flex justify-between" style={{ borderTop: "1px solid var(--color-border-default)" }}><span style={{ fontWeight: 700 }}>실제 받는 금액</span><span style={{ fontWeight: 800, color: "var(--color-action-primary)" }}>{formatForeign(quote.receiveAmount, selectedCountry.currency)} {selectedCountry.currency}</span></div>
                    </div>
                  )}
                  <button onClick={() => handleProviderClick(quote)} className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl transition-all active:scale-[0.98]" style={{ fontWeight: 600, backgroundColor: isFirst ? "var(--color-action-primary)" : "var(--color-surface-secondary)", color: isFirst ? "var(--color-text-on-color)" : "var(--color-text-primary)" }}>
                    {quote.nameKo}로 보내기 <ArrowRight size={16} />
                  </button>
                </div>
              );
            })}
        ...
        <div className="rounded-3xl p-4" style={{ backgroundColor: "var(--color-surface-primary)" }}>
          <div className="flex gap-2">
            ...
            <p className="text-[11px] leading-relaxed" style={{ color: "var(--color-text-secondary)" }}>
              환율과 수수료는 실시간 변동되며, 표시된 금액은 예상치입니다. 실제 송금 금액은 각 업체에서 최종 확인해주세요. Settle은 송금 중개가 아닌 비교 정보 서비스입니다. 순위는 실수령액 기준으로 자동 정렬되며, 제휴 여부와 무관합니다.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          <div className="relative rounded-t-3xl w-full max-w-lg max-h-[70vh] flex flex-col animate-slide-up" style={{ backgroundColor: "var(--color-surface-primary)" }}>
            <div className="flex items-center justify-between p-5 border-b" style={{ borderColor: "var(--color-border-default)" }}>
              <h2 className="text-[17px]" style={{ fontWeight: 700 }}>송금 국가 선택</h2>
            ...
            <div className="px-5 py-3">
              <div className="flex items-center gap-2 rounded-2xl px-3 py-2.5" style={{ backgroundColor: "var(--color-surface-secondary)" }}>
                ...
                <input type="text" placeholder="국가 검색..." value={countrySearch} onChange={(e) => setCountrySearch(e.target.value)} className="flex-1 bg-transparent text-[13px] outline-none" style={{ color: "var(--color-text-primary)" }} />
              </div>
            </div>
            <div className="flex-1 overflow-y-auto pb-10">
              {filteredCountries.map((country) => (
                <button key={country.code} onClick={() => { setSelectedCountry(country); setShowCountryPicker(false); setCountrySearch(""); }} className="w-full flex items-center gap-4 px-5 py-4 text-left transition-colors" style={{ borderBottom: "1px solid var(--color-border-default)", backgroundColor: selectedCountry.code === country.code ? "color-mix(in srgb, var(--color-action-primary) 5%, transparent)" : undefined }}>
                  ...
                  <div className="flex-1"><p className="text-[13px]" style={{ fontWeight: 600 }}>{country.name}</p><p className="text-[12px] mt-0.5" style={{ color: "var(--color-text-secondary)" }}>{country.nameEn}</p></div>
                  <span className="text-[13px]" style={{ fontWeight: 500, color: "var(--color-text-secondary)" }}>{country.currency}</span>
